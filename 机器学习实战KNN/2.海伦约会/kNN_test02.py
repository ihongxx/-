...
def classify0(inx,dataSet,labels,k):
    ...
    for i in range(k):
        ...
        #dict.get(key,default=None),字典的get()方法，返回指定的键，如果值不再字典中返回默认值
        #计算类别次数
...
def file2matrix(filename):
    ...
    #返回的Numpy矩阵，解析成的数据：numberOfLines行，3列
    ...
    for line in arrayOLines:
        ...
        #根据文本中标记的喜欢的程度进行分类，1代表不喜欢，2代表魅力一般，3代表具有魅力
...
def showdatas(datingDataMat, datingLabels):
    ...
    #将fig画布分隔成一行一列，不共享x轴和y轴，fig画布的大小为（13，8）
    #当nrow=2，nclos=2时，代表fig画布被分为四个区域，axs[0][0]表示第一行第一个区域
    fig, axs = plt.subplot(nrows=2,ncols=2,sharex=False,sharey=False,figsize=(13,8))
    ...
    #画出散点图，以datingDataMat矩阵的第一（飞行常客例程）、第二列（玩游戏）数据面散点数据，散点大小为15，托明度0。5
    ...
    #添加图例
    #添加图纸上每根线的说明，配合plt.plot(label = 'TextHere')使用
...
def datingClassTest():
    ...
    for i in range(numTestVecs):
        #前numTestVecs个数据作为测试集，后m-numTsetVecs个数据作为训练集
